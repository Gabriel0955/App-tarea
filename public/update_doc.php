...
$stmt = $pdo->prepare('SELECT doc_plan_prueba, doc_plan_produccion, doc_control_objeto, doc_politica_respaldo FROM tasks WHERE id = ?');
...
$update_stmt = $pdo->prepare('UPDATE tasks SET doc_plan_prueba = ?, doc_plan_produccion = ?, doc_control_objeto = ?, doc_politica_respaldo = ? WHERE id = ?');
$update_stmt->execute([
    $doc_plan_prueba,
    $doc_plan_produccion,
    $doc_control_objeto,
